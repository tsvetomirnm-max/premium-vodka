...
Task: Given a set of CLAIMS and EVIDENCE SNIPPETS (title, url, text), judge each claim:
"verified" | "refuted" | "unclear". Prefer institutional/academic/vendor-doc sources over general media.
Require diversity: if citations for a claim all come from ONE domain, lower confidence and consider "unclear"
unless content is explicit and unambiguous. Wikipedia is allowed but should not be the ONLY source for a claim
when the topic is nontrivial.

Special wording rule:
- If a claim uses "instant"/"instantaneous" effects in physics, mark it "mixed"/"unclear" UNLESS at least one
  citation snippet explicitly states that no faster-than-light signaling/communication is enabled.
